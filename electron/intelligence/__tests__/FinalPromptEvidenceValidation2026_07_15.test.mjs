...
test('resume + JD comparison requires both families in the final prompt', () => {
  ...
  const result = validateFinalPromptEvidence({
    decision, contract, pack: p, finalUserPrompt: rendered,
  });
...
test('JD-only final prompt rejects a leaked résumé family', () => {
  ...
  const result = validateFinalPromptEvidence({
    decision: jdOnlyDecision, contract, pack: p, finalUserPrompt: rendered,
  });
  assert.equal(result.ok, false);
...
test('refusal policy never proceeds to a provider packet', () => {
  ...
  const result = validateFinalPromptEvidence({
    ...
    contract, pack: p, finalUserPrompt: '<evidence_pack />',
  });
  assert.equal(result.ok, false);
