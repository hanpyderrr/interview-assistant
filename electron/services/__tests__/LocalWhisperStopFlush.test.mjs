...
const listenerStart = source.indexOf('    private attachWorkerListeners(sessionId: number): void');
...
test('LocalWhisperSTT.stop does not clear queued VAD finals before worker readiness', () => {
  ...
  assert.match(stopSource, /this\.isDrainingFinals = true;[\s\S]*segs\.forEach\(s => this\.dispatchSegment\(s\)\);/);
...
test('LocalWhisperSTT drains queued stop-time finals before terminating worker', () => {
  ...
  assert.match(
    flushPendingSource,
    /queued\.forEach\(\(\{\s*audio,\s*metadata\s*\}\)\s*=>\s*\{[\s\S]*?this\.drainingFinalsInFlight\+\+;[\s\S]*?this\.finalTaskMetadata\.set\(taskId, metadata\);[\s\S]*?this\.sendTranscribe\(audio, false, taskId\);/,
  );
...
test('LocalWhisperSTT counts flushed pending finals as in-flight before terminating', () => {
  assert.ok(flushPendingStart >= 0, 'flushPending should exist');
  assert.match(
    flushPendingSource,
    /queued\.forEach\(\(\{\s*audio,\s*metadata\s*\}\)\s*=>\s*\{[\s\S]{0,260}if \(this\.isDrainingFinals\) \{\s*this\.drainingFinalsInFlight\+\+;/,
    'flushed queued finals must increment the drain counter before sendTranscribe',
  );
});
