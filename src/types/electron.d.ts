...
export type AlibabaFunAsrModel = 'fun-asr-realtime' | 'fun-asr-realtime-2026-02-28'
...
export interface ElectronAPI {
  ...
  setSttProvider: (provider: 'none' | 'google' | 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'natively' | 'local-whisper' | 'alibaba-fun-asr' | 'local-funasr') => Promise<{ success: boolean; error?: string }>
  ...
  testSttConnection: (provider: 'groq' | 'openai' | 'deepgram' | 'elevenlabs' | 'azure' | 'ibmwatson' | 'soniox' | 'alibaba-fun-asr', apiKey: string, regionOrConfig?: string | AlibabaFunAsrPublicConfig) => Promise<{ success: boolean; error?: string }>
  testLocalFunAsrConnection: () => Promise<{ success: boolean; error?: string; health?: { state: 'loading' | 'ready' | 'failed'; status: string; detail: string; model_load_seconds?: number | null; warmup_seconds?: number | null; request_count?: number; success_count?: number; failure_count?: number; cuda_memory_current_mib?: number | null; cuda_memory_peak_mib?: number | null } }>
