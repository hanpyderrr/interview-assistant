...
**NEXT ACTION**: Read task `bbl1uvl7p`. If it passes, re-check shared git status, then run the pre-existing full 140-case real-provider benchmark as `dev-run-006-thesis079-ranking` (Codex quota precheck required; current ~31% is above threshold) and compare to true 125/140 baseline. If tests fail, repair/revert only this two-part fix; do not stack another mechanism.
