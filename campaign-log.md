...
**NEXT ACTION**: On the monitor event (or fallback wakeup), re-check that PIDs `37130`, `37146`, `38618`, and `38640` are gone. If they are gone, run exactly one prepared forensic command: `CTXOS_BENCHMARK_SPLITS=development CTXOS_BENCHMARK_CASE_IDS=THESIS-079 CTXOS_BENCHMARK_CAPTURE_RAW_RETRIEVAL=1 CTXOS_BENCHMARK_RUN_ID=thesis-079-forensic NODE_OPTIONS=--enable-source-maps node tests/context-os-real-backend/run-200q-benchmark.mjs`. If any remain, retain/re-arm the monitor and schedule another 20-minute fallback. Never kill or signal concurrent-session processes.
