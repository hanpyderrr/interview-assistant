...
**NEXT ACTION**: Design a fix for THESIS-079 as its own dedicated unit: (a) consider whether `ENTITY_TOKEN_RE` should exclude common technical acronyms functioning as adjectives (USB, GPU, CPU, RAM, LED, etc.) when not adjacent to a capitalized proper-noun pattern — needs care not to break genuine acronym-as-entity cases (e.g. "ROS", "OpenVLA"); (b) consider a secondary tie-break in `answerRelevanceScore` that weights RAW retrieval score more heavily when term-coverage is tied or near-tied, so a chunk with dramatically higher semantic/lexical retrieval confidence isn't discounted purely for using different vocabulary. Full regression (140-case benchmark + existing unit tests) required before landing either change, given the blast radius. Re-check quota and git branch/status first (shared workspace, concurrent session very active this iteration — 6+ new commits appeared, including a THESIS-060 fix, `dev-run-005-fieldproof` benchmark, and a since-superseded Electron-contention wait loop).
