...
**NEXT ACTION**: Two of the campaign's 4 landed fixes this session (THESIS-093, THESIS-131) now have a confirmed-correct evidence pipeline paired with unreliable MiniMax-M3 generation on their specific fact patterns. This is worth its own investigation track — possibly a shared root cause (both involve disambiguating between two similar/adjacent facts in the same source paragraph or section) rather than two independent nondeterminism instances. Consider as a candidate for the NEXT phase of work: either (a) a stricter answer-verification/regeneration pass specifically for questions whose evidence contains multiple similar candidate facts, or (b) generation-parameter tuning (temperature/seed), or (c) documenting this as an accepted model-quality limit and moving on. Do not attempt to fix via further evidence-pipeline changes — both pipelines are confirmed already correct.
