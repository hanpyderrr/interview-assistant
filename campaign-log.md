...
- Added production-default regression coverage for the explicit core-on/strict-off split and per-surface environment kill switches. Also corrected the generic rollout test so it validates sibling *documented defaults* rather than incorrectly assuming every sibling is false now that Context OS core flags are intentionally true by default.
- Deterministic validation is clean: `npm run build:electron`, `npm run typecheck:electron`, and 69 targeted flag/gate/manual-chat/WTA/rollout tests all pass. `git diff --check` is clean.
- Live production-mode E2E proof remains deferred: the shared pool still reports Gemini `429` and MiniMax `502`/`429`, so it cannot distinguish a production rollout behavior from provider failure. No provider-dependent benchmark was run or scored.
