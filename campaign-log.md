...
**NEXT ACTION:** Probe the authenticated local MiniMax SSE route once. If it responds, rerun only the 20 provider-empty case IDs from `dev-run-007` under a fresh run ID to distinguish transient provider outage from persistent benchmark failure. Do not change ranking or provider timeout code. If that subset is provider-clean, use it only to recover unavailable samples, then schedule a new full 140-case run for a clean baseline comparison; if it is not clean, checkpoint provider instability and defer the full comparison.
