...
**NEXT ACTION:** Wait for the in-flight retrieval-evidence design review, then choose a genuinely independent, generic topic/property-consistency signal for THESIS-079 that preserves the restored Mercury-controller regression. Add adversarial unit coverage for both scenarios, live-trace the exact selection decision, and only then run the focused real THESIS-079 benchmark. Do not revisit fixed raw-score weights or run another 140-case benchmark until the focused trace is green.
