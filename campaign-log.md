...
**NEXT ACTION**: Retry `golden-trace-h4-routing-deadzone.mjs` (single "conversational" question only, with `NATIVELY_CONTEXT_OS_PROVIDER_CAPTURE=1` re-added for full payload capture) once shared-workspace Electron contention clears, to determine whether `documentGroundedCustomModeActive` was false/null at the moment of the live failure (a mode-info propagation bug) or the guard itself silently didn't fire despite correct inputs (a guard-logic bug) — the fix target differs significantly between these two explanations. Re-check quota and free memory before the next Electron-dependent work.
