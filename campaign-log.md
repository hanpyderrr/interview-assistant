...
**NEXT ACTION:** Commit the focused THESIS-079 property-bound ranking repair plus deterministic tests and this checkpoint. Then diagnose/recover the real provider route (Gemini quota exhausted; local Natively `/v1/chat` connect timeout) without changing ranking code. Once a provider can return a response, re-run exactly `THESIS-079` to confirm `Logitech C920`, then run the 140-case regression only after that focused run is green.
