...
**NEXT ACTION**: Bring the production-flag decision to the user directly (this is a judgment call, not a bug fix). Then attempt a live TurnEvidenceCoordinator verification trace: create a mode with BOTH a résumé/profile and a reference file attached, ask a profile-and-document mixed question, and confirm via `NATIVELY_CONTEXT_OS_PROMPT_AUDIT`/trace flags that the coordinator's multi-family pack actually governs the turn (not just that its unit tests pass in isolation).
