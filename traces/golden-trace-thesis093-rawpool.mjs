...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace Thesis093RawPool', templateType: 'lecture' });
  return c.mode;
});
...
const inspection = await R('__e2e__:inspect-retrieval', { modeId: mode.id, query: 'What two objects are visible but never interacted with?', forceDocumentGrounding: true });

console.log('RAW_POOL_BEGIN');
console.log(JSON.stringify({ blockLength: inspection?.blockLength, block: inspection?.block }, null, 2));
console.log('RAW_POOL_END');
