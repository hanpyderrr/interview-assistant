...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace Thesis093Detail', templateType: 'lecture' });
  return c.mode;
});
...
const response = await askManual('What two objects are visible but never interacted with?');

const audit = await R('__e2e__:context-os-prompt-audit');
const benchAudit = await R('__e2e__:context-os-benchmark-audit');
...
console.log('GOLDEN_TRACE_THESIS093_DETAIL_BEGIN');
console.log(JSON.stringify({ response, audit, benchAudit, captureCount: capture.length }, null, 2));
console.log('GOLDEN_TRACE_THESIS093_DETAIL_END');

console.log('PROVIDER_PAYLOAD_BEGIN');
console.log(JSON.stringify(capture, null, 2));
console.log('PROVIDER_PAYLOAD_END');

// Also dump evidence-selection trace lines from main process logs
const selLines = mainLogs.join('').split('\n').filter((l) => l.includes('TRACE:evidence-selection') || l.includes('EvidenceResolver') || l.includes('selectSmallestSufficient'));
