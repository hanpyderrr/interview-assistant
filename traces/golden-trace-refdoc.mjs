// GOLDEN TRACE #1 (grounding campaign, Phase 0): real-Electron, real-fixture
// verification of the founder's symptom #1 — false refusal on the FIRST
// question after attaching a reference document, even when the answer is
// verbatim in the file. Also probes symptom #2 (nondeterminism across timing).
//
// Drives the real app through dist-electron/electron/main.js, same as
// tests/e2e-modes/_ks_realfixture_verify.mjs. Uses __e2e__:add-reference-file
// (content-based ingest through the REAL ModesManager.addReferenceFile +
// indexReferenceFile — same code the renderer's upload flow calls after
// parsing) so we control exact timing between attach and ask.
...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace RefDoc Lecture', templateType: 'lecture' });
  return c.mode;
});
...
const askAt = (label, delayMs) => async () => {
  ...
  const capture = await RAW(async () => {
    const api = window.electronAPI || window.api;
    return api.e2eInvoke ? await api.e2eInvoke('__e2e__:context-os-benchmark-audit') : null;
  }).catch(() => null);
  return {
    label,
...
console.log('GOLDEN_TRACE_REFDOC_BEGIN');
console.log(JSON.stringify(verdict, null, 2));
console.log('GOLDEN_TRACE_REFDOC_END');
await app.close();
