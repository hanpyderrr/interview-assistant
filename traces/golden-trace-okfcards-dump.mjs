...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace OkfCardsDump2', templateType: 'lecture' });
  return c.mode;
});
...
const cards = await R('__e2e__:dump-okf-cards', { modeId: mode.id, titles: ['Conversational Agent', 'Technical Specifications', 'Agentic AI Framework implementation'] });

console.log('OKF_CARDS_BEGIN');
console.log(JSON.stringify(cards, null, 2));
console.log('OKF_CARDS_END');
