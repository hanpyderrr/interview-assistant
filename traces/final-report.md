...
| H6 — Mercury X1 hardware-spec-table vs. AI-framework-layer entity confusion (THESIS-042/060): the model answers with a real-but-wrong fact about the SAME entity instead of the specifically-asked-about fact | Manual full-text verification against the source PDF | Root cause not yet pinned to a specific mechanism (likely table-vs-prose chunking/embedding precision) — needs its own investigation, different from the cap issue. |
