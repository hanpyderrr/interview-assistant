...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace Thesis129RawPool', templateType: 'lecture' });
  return c.mode;
});
...
const inspection = await R('__e2e__:inspect-retrieval', { modeId: mode.id, query: 'What model is the visual backbone for the Self-Awareness Tool?', forceDocumentGrounding: true });

console.log('RAW_POOL_129_BEGIN');
console.log(JSON.stringify({ blockLength: inspection?.blockLength, block: inspection?.block }, null, 2));
console.log('RAW_POOL_129_END');
