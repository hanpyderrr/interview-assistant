...
const mode = await RAW(async () => {
  ...
  const c = await api.modesCreate({ name: 'GoldenTrace Thesis131Full', templateType: 'lecture' });
  return c.mode;
});
...
const response = await askManual('What camera perspective does the Self-Awareness Tool use?');
...
console.log('THESIS131_FULL_BEGIN');
console.log(JSON.stringify({ response, capture }, null, 2));
console.log('THESIS131_FULL_END');
