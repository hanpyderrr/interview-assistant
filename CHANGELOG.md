    ...
    - **Senior-review fixes for provider state-awareness + JD/resume ingest space stamp (2026-07-12)**: Follows the senior code-review pass on the provider state-awareness commit and closes every MEDIUM/LOW finding before shipping. (1) **`EmbeddingPipeline.getEmbeddingWithFallback()` exposes producer metadata end-to-end** — `main.ts` wires the orchestrator's `setEmbedFn` through the new metadata-returning call (was the bare `getEmbedding()` that silently promoted and returned just a vector), so premium `KnowledgeOrchestrator.ingestDocument` can stamp JD/resume/reference-file nodes with the *actual* producing space rather than a pre-call snapshot. The snapshot-before-embed path now correctly inherits the resolver's `space/provider/dimensions`, so a cloud→local promotion mid-ingest can no longer cross-stamp a node with the old cloud space. `electron/main.ts` registers both `setEmbedFn` (legacy bare-vector callers) and `setEmbedWithMetadataFn` (orchestrator callers) — single plumbing point, no API churn for the existing legacy callers. (2) **`PRAGMA foreign_keys = ON` blast-radius covered end-to-end** — the FK cascade suite now asserts the full `mode_reference_files → knowledge_sources → knowledge_packs → knowledge_cards → knowledge_card_versions + knowledge_entities/relations/index_versions` graph cascades on `deleteReferenceFile()` AND that a bare `deleteMode()` cleans up `modes / mode_reference_files / mode_note_sections / knowledge_sources / knowledge_packs`. The pragma is enabled unconditionally in `DatabaseManager.initialize()` on the shared connection (no longer depends on premium loading), so the source-available build cannot regress to orphan-only child rows. (3) **Groq/OpenAI model-id disambiguation in `modelAvailable`** — the catch-all `modelId.includes('openai')` previously misclassified Groq-hosted ids like `openai/gpt-oss-120b` as needing an OpenAI key. The Groq branch (`llama-`, `mixtral-`, `gemma-`, `meta-llama/`, `qwen/`) now matches BEFORE the broad OpenAI catch-all, and a Groq-specific branch is explicitly included alongside `gpt-/o1-/o3-` so the OSS family stays correctly gated by the Groq key only. (4) **`validateUrlForSsrf` IPv6 + encoded-IP guard hardening** — the OpenAI-compatible STT base URL validator (`validateSttBaseUrl`) and the cURL validator now block IPv6 loopback (`::1`), IPv6 unique-local (`fc00::/7` covers `fc*` and `fd*`), IPv6 link-local (`fe80::/10`), and pure-numeric / `0x`-prefixed hostnames (`https://2130706433/`, `https://0x7f000001/`) that some stacks silently re-interpret as `127.0.0.1` after DNS canonicalization. Test coverage added for all six classes. (5) **`ChatTrace` clarification short-circuit alignment** — when Context OS resolves `sourceOwner='clarify'` (general/ambiguous modes where more than one source universe could own an ambiguous noun), the manual-chat stream now logs the EXACT `finalAction` (`'clarify'`) instead of a hardcoded `'answer'`, so the trace ring can no longer show a `sourceOwner=clarify` line paired with an `answer` outcome that never happened. (6) **LiveRAGIndexer + ManualChat integration now re-uses `EvidenceResolver`'s typed pack** for both the prompt and the post-stream validator/claim-verifier — closes the second-retrieval class of bug where the validator saw evidence the answer was never grounded in. (7) Diagnostic kill-switches (`NATIVELY_NO_LOCAL_MODELS`, `NATIVELY_DISABLE_PHONE_MIRROR`, `NATIVELY_OPEN_DEVTOOLS`, GPU feature-status dump) were removed from the active tree now that the leak investigation on `electron-43` landed its real fixes. **Regression suites**: `electron/rag/__tests__/KnowledgeIngestSpaceMetadata.test.mjs` (producer-metadata plumbing through orchestrator → ingest), `electron/rag/__tests__/KnowledgeReembedIntegration.test.mjs` (real compiled orchestrator method against real DB for the v1→v2 re-embed path), `electron/db/__tests__/ForeignKeyCascade2026_07_10.test.mjs` (extended to cover mode + reference-file cascade chains), and `electron/utils/__tests__/sttEndpointValidation.test.mjs` (encoded-IP + IPv6 ULA/link-local coverage). 71/71 focused tests pass; `npm run typecheck:electron` and `npm run build:electron` clean.
